#!/usr/bin/env python3
# VaVAM-B 12i v2 — Thor integrated GPU-resident benchmark
#
# Measures:
#   1) Prefill FP16
...
REFERENCE_NPZ = ROOT / "ONNX" / "vavam_joint_kv_prefill_B_v10_addmask_reference.npz"
...
def compile_cuda_module():
    ...
    result = nvrtc.nvrtcGetPTX(program)
    err = result[0] if isinstance(result, tuple) else result
...
def make_euler_args(action_ptr, velocity_ptr, dt):
    action_arg = ctypes.c_void_p(int(action_ptr))
    velocity_arg = ctypes.c_void_p(int(velocity_ptr))
    dt_arg = ctypes.c_float(float(dt))

    return (
        ctypes.c_void_p(ctypes.addressof(action_arg)),
        ctypes.c_void_p(ctypes.addressof(velocity_arg)),
        ctypes.c_void_p(ctypes.addressof(dt_arg)),
    )


def make_t_args(t_ptr, dt):
    t_arg = ctypes.c_void_p(int(t_ptr))
    dt_arg = ctypes.c_float(float(dt))

    return (
        ctypes.c_void_p(ctypes.addressof(t_arg)),
        ctypes.c_void_p(ctypes.addressof(dt_arg)),
    )
...
def main():
    print("=" * 80)
    print("VaVAM-B 12i v2 — Thor GPU-Resident E2E Benchmark")
    print("=" * 80)
