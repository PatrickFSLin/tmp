...
def safe_name(text):
    """Make a filesystem-safe short name for plot files."""
    text = str(text)
...
def plot_trajectories(
    ...
    """Save one XY trajectory plot for one nuScenes evaluation window."""
    import matplotlib
    matplotlib.use("Agg")
    ...
    # Plot all 10 candidates so the stochastic diversity is visible.
    for i, traj in enumerate(trajectories):
        ax.plot(
            ...
            linewidth=1.0,
            alpha=0.45,
            label=f"Pred {i} (ADE {ades[i]:.2f}m)" if i < 3 else None,
        )

    # Highlight the best candidate in each nested set.
    best_indices = {
        1: int(np.argmin(ades[:1])),
        4: int(np.argmin(ades[:4])),
        10: int(np.argmin(ades[:10])),
    }
    line_specs = [
        (1, best_indices[1]),
        (4, best_indices[4]),
    ...
    for m, idx in line_specs:
        traj = trajectories[idx]
        ax.plot(
            traj[:, 0],
            traj[:, 1],
            linewidth=2.5,
            label=f"Best M={m} (idx {idx}, minADE {ades[idx]:.3f}m)",
        )
    ...
    ax.scatter([0.0], [0.0], marker="o", s=35, label="Ego start")

    ax.set_xlabel("X [m]")
    ax.set_ylabel("Y [m]")
    ax.set_title(
        f"VaVAM-B Thor | sample={sample_index} | {scene} | window={window_idx}\n"
        f"M1={ades[0]:.3f}m, M4={np.min(ades[:4]):.3f}m, "
        f"M10={np.min(ades[:10]):.3f}m\n"
...
def main():
    ...
    parser.add_argument(
        "--plot-dir",
        default=str(ROOT / "thor_eval" / "trajectory_plots"),
    )
    parser.add_argument(
        "--summary-csv",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Store all 10 trajectories in the NPZ; default is also all trajectories.",
    )
    ...
    try:
        for local_idx, sample_index in enumerate(range(args.start_index, end_index)):
            ...
            runner.run_prefill(visual_tokens, command)
            ...
            plot_name = (
                ...
                f"{safe_name(scene)}_w{window_idx:04d}_"
                f"{safe_name(Path(first_frame).stem)}.png"
            )
            plot_path = plot_dir / plot_name
            # Plot only available candidates; for the requested run this is 10.
            plot_trajectories(
                gt,
                trajectories,
                ades,
    ...
    np.savez_compressed(
        ...
        first_frames=np.asarray(first_frames, dtype="<U512"),
        last_frames=np.asarray(last_frames, dtype="<U512"),
        plot_paths=np.asarray(plot_paths, dtype="<U1024"),
    ...
    print()
    print("[OK] Full Thor accuracy evaluation completed.")
