...
def normalize_steps(name, arr):
    """Normalize captured traces while preserving numerical values.

    Action traces may be either:
      (10, 1, 6, 2)       -> [step, batch, T, xy]
      (10, 1, 1, 6, 2)    -> [step, batch, M=1, T, xy]

    For M=1, the extra singleton mode dimension is removed.
    diffusion_t is normalized to (10, 1).
    """
    arr = np.asarray(arr)
    ...
    if name.endswith("diffusion_t.npy"):
        ...
        if arr.shape == (NUM_STEPS,):
            return arr[:, None]
        # Accept any (10, ...) shape where all dimensions after step are 1.
        if arr.ndim >= 2 and arr.shape[0] == NUM_STEPS and all(d == 1 for d in arr.shape[1:]):
            return arr.reshape(NUM_STEPS, 1)
        raise RuntimeError(
            f"Unexpected {name} shape: {arr.shape}; "
            "expected (10,1), (10,), or trailing-singleton equivalent"
        )

    if arr.shape == (NUM_STEPS, 1, 6, 2):
        return arr

    if arr.shape == (NUM_STEPS, 1, 1, 6, 2):
        # Remove ONLY M=1. No numerical conversion.
        return arr[:, :, 0, :, :]

    raise RuntimeError(
        f"Unexpected {name} shape: {arr.shape}; "
        "expected (10,1,6,2) or (10,1,1,6,2)"
    )
